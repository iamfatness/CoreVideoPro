...
ShowPreparationPlan plan(std::uint64_t base = 0) {
  return {{"show", base, 1}, "plan-" + std::to_string(base + 1), base + 1,
    {{ShowPreparationRequirement::Kind::SourceSubscription, "camera", 2},
     {ShowPreparationRequirement::Kind::GpuResource, "canvas", 1},
     {ShowPreparationRequirement::Kind::OutputReservation, "recorder", 3}}};
}
...
TEST(ShowPreparationTransaction, RequiresAllResourcesAndPollReplayDoesNotPrepareTwice) {
  ...
  ASSERT_TRUE(done.applied);
  EXPECT_EQ(done.status, Status::Applied);
  EXPECT_EQ(done.applied->plan->revision, 1ULL);
  EXPECT_EQ(done.applied->leases.size(), 3U);
  EXPECT_EQ(p->calls["camera"], 1);
  const auto calls = p->calls;
  EXPECT_EQ(owner.poll().applied.get(), done.applied.get());
...
TEST(ShowPreparationTransaction, FailurePreservesPreviouslyAppliedRevision) {
  ...
  ShowPreparationTransaction owner({"show", 0, 1}, p);
  owner.begin(plan()); const auto original = owner.poll().applied;
...
TEST(ShowPreparationTransaction, AuthorityGenerationChangeFencesInFlightReadiness) {
  ...
  EXPECT_FALSE(owner.advanceAuthority({"show", 0, 1}, {"show", 1, 1}));
}
...
TEST(ShowPreparationTransaction, InputCopyIsImmutableAndStateAdmissionIsBounded) {
  ...
  const auto done = owner.poll(); ASSERT_TRUE(done.applied);
  EXPECT_EQ(done.applied->plan->requirements[0].generation, 2ULL);
...
TEST(ShowPreparationTransaction, ConcurrentPollAttemptDoesNotDuplicatePreparation) {
  ...
  EXPECT_EQ(owner.poll().status, Status::Applied);
