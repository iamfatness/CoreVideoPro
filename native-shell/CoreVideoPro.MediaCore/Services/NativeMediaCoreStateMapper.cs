...
public static class NativeMediaCoreStateMapper
{
        ...
        NativeMediaCoreWireState wire)
    {
        ...
        programFrameCount = Math.Max(programFrameCount, baseSnapshot.ProgramFrameCount);
        ...
        var compositorStatus = programFrameCount > 0
            ...
            : compositorRenderer.Equals("software", StringComparison.Ordinal)
                ? baseSnapshot.Compositor.Status
                : "idle";
        ...
        var programFrame = baseSnapshot.ProgramFrame is not null
            ? baseSnapshot.ProgramFrame with
            {
                FrameNumber = programFrameCount > 0 ? programFrameCount : baseSnapshot.ProgramFrame.FrameNumber,
                RenderPlanId = wire.RenderPlanId ?? baseSnapshot.ProgramFrame.RenderPlanId
            }
            : programFrameCount > 0
