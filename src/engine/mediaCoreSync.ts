...
export class InMemoryMediaCoreSyncEngine implements MediaCoreSyncEngine {
  ...
  private syncZoomSourceLifecycleEvents(sources: NativeMediaCoreZoomSource[], elapsedMs: number) {
    ...
    nextStates.forEach((state, participantId) => {
      ...
      if (!previous) {
        this.addEvent("info", "source", "Zoom participant joined", `${state.displayName} joined ${state.breakoutRoomName}.`, undefined, participantId, elapsedMs);
        return;
      }

      zoomSourceLifecycleEvents(previous, state).forEach((event) => {
        this.addEvent(event.severity, "source", event.title, event.detail, undefined, participantId, elapsedMs);
      });
    });

    this.zoomSourceLifecycleStates.forEach((state, participantId) => {
      if (!nextStates.has(participantId)) {
        this.addEvent("info", "source", "Zoom participant left", `${state.displayName} left ${state.breakoutRoomName}.`, undefined, participantId, elapsedMs);
      }
    });
...
function zoomSourceLifecycleState(source: NativeMediaCoreZoomSource): ZoomSourceLifecycleState {
  return {
    participantId: source.participantId,
    displayName: source.displayName,
...
function zoomSourceLifecycleEvents(previous: ZoomSourceLifecycleState, current: ZoomSourceLifecycleState) {
  const events: Array<Pick<NativeMediaCoreEvent, "severity" | "title" | "detail">> = [];
