...
- Real frame-pixel transport (F1) for the deterministic test-pattern source: a bounded, ref-counted `FramePool` recycles BGRA buffers with dropped-frame back-pressure, the in-stub `TestPatternCaptureDevice` publishes populated-pixel `VideoFrame`s through `pollVideoFrames()`, and both the CPU (`CpuNoopCompositor`) and dev-gated D3D11 (`uploadLayerTexture`) compositors upload those real pixels into the `ProgramFrame`. Capture snapshots report real `framesIngested`/`droppedFrames` counters mirrored across the C++ and TypeScript protocols, with a decoder-stage seam (`IVideoDecoder`) defined for compressed sources behind a dev gate (no codec implemented yet).
...
- Optional Claude-backed AI-director intelligence service (`services/ai-director/`) that consumes richer signals (speaker turns, sentiment, screen-share content, engagement) and proposes a scene via the latest Opus model; the renderer's `AiProductionEngine` calls it through the `DirectorIntelligenceService` contract with a heuristic fallback on timeout/error, the always-on heuristic stabilizer still gates every proposal (confidence thresholds + anti-thrash holds), it is gated behind a feature flag plus the `setAndForget` entitlement with the service URL/key from host config (never committed), content signals are privacy-redacted unless the operator opts in, and proposal-vs-taken telemetry is logged for tuning.
