/**
 * PCM DSP kernels — Node native-core copy of the `AudioDsp.h` F2 audio DSP core.
 *
 ...
 * LUFS, and a brickwall look-ahead peak limiter) so the native-core audio-mix
 * session can report MEASURED metrics from a synthetic signal.
 *
 * The renderer keeps its own copy in `src/engine/audioDspKernels.ts`; the two
 * workspaces do not share a package, so this file is an intentional duplicate.
 */
