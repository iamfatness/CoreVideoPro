...
2. **Owner: rule on the on-air look of a warming / stalled / failed source** (one sentence per state). Unblocks item 4, which is small once decided (the compositor already has `retainedProgram`/held-texture machinery and the loud placeholder logging).
