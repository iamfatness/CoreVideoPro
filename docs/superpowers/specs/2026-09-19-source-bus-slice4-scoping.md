...
1. **`IMediaFrameSource`** → needed **slice 3b** (media request state set at command time, hand-off inside the source), which was the same decision as the Take-semantics ruling (#449 step 1). **SATISFIED — slice 3b merged 2026-09-21 in [#567](https://github.com/iamfatness/CoreVideoPro/pull/567)** (spec `docs/superpowers/specs/2026-09-20-source-bus-slice3b-media-source-state-design.md`): `core::MediaTransports` owns one entry per `media:`/`background:` source id, fed a desired set at command time and decided by the pure `MediaTransportPolicy.h`; the cue hand-off and the `preview:` poster namespace are DELETED (a cued clip entering Program resumes its own decoder in place), and media VIDEO is polled off the bus with no `layers` argument. **The media-AUDIO prerequisite is satisfied too, by #583 (merged 2026-09-21):** media PCM no longer leaves through a direct `MediaTransports::popAudio` pop — `core::ingestSourceAudio` (`native/src/core/SourceAudioIngress.h`) drains it and stages each frame onto its `MediaAssetSource`, and `SourceBus::ingestAudio` is what emits it, counted per source. So `IMediaFrameSource` now survives ONLY as the per-entry DECODER contract (`ModuleSet::mediaDecoderFactory`), and retiring it is a rename/reshape of that contract rather than a pipeline move. #449 **step 1** (hold the outgoing picture on a cold cut) is still open, and is not a prerequisite for the retirement.
...
1. **Owner: rule on #449 step 1** (hold outgoing picture on a plain cut / go-live semantics). Unblocks 3b (media) and defines the hand-off; nothing else in #535 should move media before it. **DONE 2026-09-19/20 for the go-live half — the core source owns play/pause state and Take drives it — and slice 3b shipped on that ruling, merged in #567 (see item 1 above). Step 1 itself, holding the outgoing picture on a COLD cut, is still open and still its own slice.**
...
5. Retire `IMediaFrameSource` after 3b + media audio. **Both prerequisites are now met — 3b merged in #567, media PCM onto the bus in #583 — so this is unblocked; what remains is reshaping the per-entry decoder contract itself.**
