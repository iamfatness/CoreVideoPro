...
|---|---|
| [#582](https://github.com/iamfatness/CoreVideoPro/issues/582) | **Confirmed by owner in Preview and multiview during a speaker switch** on beta `4dfe476`. Disabling SDK render post-processing shipped in #585 but did not eliminate the visible flash or measured incoming-frame brightening. Next: correlate the SDK's per-frame range flag with the short brightness excursion. Root cause remains unproven; separate darkening artifacts are not this acceptance target. |
