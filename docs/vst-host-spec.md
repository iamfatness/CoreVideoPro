...
| P3 | **Channel** inserts (SHIPPED — `AudioDsp.h` routes channel + bus chains through the host) + generic parameter surface (slider list from `params[]`) + state persistence in production prefs | Param moves are audible + survive restart | **SHIPPED** (round-2 PR 2): `IEditController` raw ABI (`vst-abi.h`, layout static_asserts). Host publishes the active selection's param surface (first 64 by index + total count, ASCII-folded title/units/display) and accepts set-param over a dedicated latest-wins ring + event; core snapshot `pluginHost.serve.params[]` is rebuilt only on a param-generation change (never per tick). Shell renders generic sliders in the insert flyout (rebuilt on Opening — transient-flyout pattern). State: `IComponent::get/setState` over a raw-ABI memory `IBStream`; get-state (pull command → base64) + set-state over a single-shot 1 MiB block area (larger states loud-fail with their size). Blobs persist per selection in `ProductionOutputPreferences` **v6** and are re-pushed into every host generation, including after each **respawn** (closes "respawn loses state"). `--state-roundtrip <bundle> <class>` CLI proof mode. All param/state traffic rides separate events — the 4 ms audio exchange is never stalled. |
| A3 | **Plugin latency compensation** (round-2 PR 2) | `latencySamples` telemetry + per-insert badge; delay-aligned mix; PTS invariance tests | **SHIPPED (partial scope, honest)**: `latencySamples` carried in the block → `pluginHost.serve.{latencySamples,latencyMs}` telemetry + per-insert "+N.N ms" badge (all plugins). Mix alignment: CHANNEL-level compensating delay lines (`AudioDsp.h applyCompensatingDelay`, declick ramp on change) delay non-plugin-hosting sibling channels to the active plugin latency, default-ON behind `COREVIDEO_VST_LATENCY_ALIGN`. The added latency rides the shared-epoch PTS clock (`RecordingPtsClock` latches it at the first audio buffer, clamped to the epoch) so recordings stay A/V-synced. **Deferred:** cross-BUS latency attribution (per-path, not the single active-selection value) — the single-slot telemetry can't express it yet; documented in `master-vst-round2-spec.md` §A3. |
